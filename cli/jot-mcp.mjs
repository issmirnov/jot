...
if (!INSTANCE_NAME) {
  log("JOT_INSTANCE env var is required (e.g. JOT_INSTANCE=smirnov).");
  log("Register an instance first with: jot register <name> <baseUrl> <token>");
  process.exit(1);
}

// Validate at startup that the instance exists AND is an owner instance.
// Token is re-resolved per tool call (below) so rotation works without restart.
{
  const probe = getInstance(INSTANCE_NAME);
  if (isShareInstance(probe)) {
    log(`Instance "${INSTANCE_NAME}" is a share-link instance.`);
    log("MCP v1 only exposes owner endpoints; register an API-key instance.");
    process.exit(1);
  }
}

function instance() {
  return getInstance(INSTANCE_NAME);
}
function baseUrl() {
...
const tools = {
  ...
  read_note: {
    ...
    handler: async ({ id }) => {
      const payload = await request(instance(), "GET", `/api/notes/${encodeURIComponent(id)}`);
      stale.record(id, payload.note.updatedAt);
      return {
        id: payload.note.id,
  ...
  create_note: {
    ...
    handler: async ({ title, markdown }) => {
      const created = await request(instance(), "POST", "/api/notes");
      await request(instance(), "PUT", `/api/notes/${encodeURIComponent(created.note.id)}`, {
        title,
        markdown,
      });
      return {
  ...
  update_note: {
    ...
    handler: async ({ id, title, markdown, shareAccess }) =>
      mu.withNote(id, async () => {
        ...
        // Wholesale update invalidates any stored read; force re-read before next edit_note.
        stale.forget(id);
  ...
  share_note: {
    ...
    handler: async ({ id, access }) =>
      mu.withNote(id, async () => {
        await request(instance(), "PUT", `/api/notes/${encodeURIComponent(id)}`, {
          shareAccess: access,
        });
        const fresh = await request(instance(), "GET", `/api/notes/${encodeURIComponent(id)}`);
        stale.forget(id);
